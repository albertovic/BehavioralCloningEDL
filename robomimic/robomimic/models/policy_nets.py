...
class EDLActorNetwork(ActorNetwork):
    """
    Actor network that predicts the parameters of a Normal-Inverse-Gamma (NIG) distribution
    ...
    def __init__(
        ...
        super(EDLActorNetwork, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            mlp_layer_dims=mlp_layer_dims,
            goal_shapes=goal_shapes,
    ...
    def _get_output_shapes(self):
        """
        Tells @MIMO_MLP superclass about the output dictionary that should be generated
        at the last layer. Network outputs parameters of NIG distribution.
        """
        return OrderedDict(
            gamma=(self.ac_dim,),   # Predicted Mean
            v=(self.ac_dim,),       # Evidence (v > 0)
            alpha=(self.ac_dim,),   # Shape (alpha > 1)
            beta=(self.ac_dim,),    # Scale (beta > 0)
        )

    def forward_train(self, obs_dict, goal_dict=None):
        """
        Computes the forward pass and returns all 4 parameters of the 
        Normal-Inverse-Gamma (NIG) distribution. 
        
        -> Used during training to calculate the EDL Loss.
        -> Used during evaluation by your custom safety trigger to calculate uncertainty.
        """
        # MIMO_MLP handles visual features and joint state concatenation
        out = MIMO_MLP.forward(self, obs=obs_dict, goal=goal_dict)
        
        # Gamma: The predicted mean (the action). Squashed to [-1, 1] for the robot.
        gamma = torch.tanh(out["gamma"])

        # Nu (v): The virtual observation count (evidence). Must be > 0.
        v = F.softplus(out["v"]) + 1e-6
        
        # Alpha: The shape parameter. Must be > 1 so variance is well-defined.
        alpha = F.softplus(out["alpha"]) + 1.1 
        
        # Beta: The scale parameter. Must be > 0.
        beta = F.softplus(out["beta"]) + 1e-6
    ...
    def forward(self, obs_dict, goal_dict=None):
        """
        Standard forward pass for Robomimic compatibility.
        Returns strictly the action tensor.
        
        """
        nig_params = self.forward_train(obs_dict, goal_dict)
        return nig_params["gamma"]
