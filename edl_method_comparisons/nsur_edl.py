...
def nsur_loss(y, mu, lambda_, alpha, beta, *, reg_weight=1e-4):
    ...
    nsur_reg = (0.5 * error ** 2 * inverse_uncertainty).mean()
...
for epoch in range(config.epochs):
    ...
    loss_tr, nll_tr, reg_tr = nsur_loss(y_tr, mu, lambda_, alpha, beta, reg_weight=reg_w)
    ...
    with torch.no_grad():
        ale_var_mean = (beta / (alpha - 1)).mean().item()
        epi_var_mean = (beta / (lambda_ * (alpha - 1))).mean().item()

    model.eval()
    with torch.no_grad():
        mu_v, lam_v, a_v, b_v = model(x_val)
        loss_v, nll_v, reg_v = nsur_loss(y_val, mu_v, lam_v, a_v, b_v, reg_weight=reg_w)
    ...
    wandb.log(
        {
            ...
            "train/ale_var_mean": ale_var_mean,
            "train/epi_var_mean": epi_var_mean,
...
model.eval()
with torch.no_grad():
    mu_tr_f, lam_tr_f, a_tr_f, b_tr_f = model(x_tr)
    loss_tr_f, nll_tr_f, reg_tr_f = nsur_loss(y_tr, mu_tr_f, lam_tr_f, a_tr_f, b_tr_f, reg_weight=reg_w)

    mu_vf, lam_vf, a_vf, b_vf = model(x_val)
    loss_v_f, nll_v_f, reg_v_f = nsur_loss(y_val, mu_vf, lam_vf, a_vf, b_vf, reg_weight=reg_w)
...
print(f"ID EAR (Epi/Ale ratio):  {id_ear:.4f}  <-- Should be low")
print(f"OOD EAR (Epi/Ale ratio): {ood_ear:.4f}  <-- Should be high")
print(f"ID Coverage (2σ):        {id_coverage:.1f}%  <-- Ideal ~95%")
