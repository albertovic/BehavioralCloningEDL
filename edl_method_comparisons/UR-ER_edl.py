...
def urer_loss(y, gamma, lambda_, alpha, beta, *, reg_weight=1e-4):
    ...
    eps = 1e-8
    reg = (-error * torch.log(torch.exp(alpha - 1.0) - 1.0 + eps)).mean()
...
for epoch in range(config.epochs):
    ...
    loss_tr, nll_tr, reg_tr = urer_loss(
        y_tr, gamma, lambda_, alpha, beta, reg_weight=reg_w
    )
    ...
    with torch.no_grad():
        ...
        loss_v, nll_v, reg_v = urer_loss(
            y_val, mu_v, lam_v, a_v, b_v, reg_weight=reg_w
        )
...
with torch.no_grad():
    ...
    loss_tr_f, nll_tr_f, reg_tr_f = urer_loss(
        y_tr, mu_tr_f, lam_tr_f, a_tr_f, b_tr_f, reg_weight=reg_w
    )
    mu_vf, lam_vf, a_vf, b_vf = model(x_val)
    loss_v_f, nll_v_f, reg_v_f = urer_loss(
        y_val, mu_vf, lam_vf, a_vf, b_vf, reg_weight=reg_w
    )
