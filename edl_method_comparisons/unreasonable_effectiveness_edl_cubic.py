...
# Data Generation (Cubic No Internal Gap)
...
x_tr, y_tr, x_val, y_val = make_cubic_split(config.total_points, config.noise_std, config.val_fraction, config.seed, config.x_range)
...
def ue_loss(y, mu, lambda_, alpha, beta, *, reg_weight=1e-4):
    ...
    reg = (torch.abs((y - mu) / (w_st + 1e-12)) * (lambda_ + 2 * alpha)).mean()
...
for epoch in range(config.epochs):
    ...
    loss_tr, nll_tr, reg_tr = ue_loss(y_tr, mu, lam, alp, bet, reg_weight=reg_w)
...
torch.save(model.state_dict(), "best_ue_cubic_model.pt")
...
wandb.run.summary.update({
    ...
    "metrics/std_peak_ood_epi": peak_ood_epi_std,
    "metrics/std_ood_ear": ood_ear_std,
    "metrics/ue_peak_ood_epi": peak_ood_epi_ue,
    "metrics/ue_ood_ear": ood_ear_ue,
})
